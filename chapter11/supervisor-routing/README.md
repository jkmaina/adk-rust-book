...
The local `0.8.0` graph story is broader than this single crate: `GraphAgent` now gives you a higher-level wrapper for graph workflows. This example intentionally stays with `StateGraph` and `AgentNode` so the routing loop, state channels, and stop conditions remain visible to the reader.
