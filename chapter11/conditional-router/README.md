...
Local `0.8.2` tightened `LlmConditionalAgent` behavior so overlapping route labels resolve deterministically. That makes this example more than a toy classifier: it shows a routing primitive you can reason about when specialist labels and fallback behavior both matter.
