...
fn main() {
    ...
    println!(
        "Main: Arc strong count after create = {}",
    ...
    for i in 1..=3 {
        ...
        let handle = thread::spawn(move || {
            ...
            println!(
                ...
                i,
                Arc::strong_count(&model_clone)
            );
        });
        handles.push(handle);
        println!(
            "Main: spawned thread {}, Arc strong count = {}",
            i,
            Arc::strong_count(&model)
        );
    }
    ...
    println!(
        "Main: Arc strong count after join = {}",
        Arc::strong_count(&model)
    );
    println!(
        "All done — the same `GeminiModel` instance was shared safely across threads using `Arc`."
    );
}
