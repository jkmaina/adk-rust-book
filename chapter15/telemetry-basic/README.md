...
In local `0.8.0`, telemetry is part of the `standard` tier rather than the default `minimal` surface. This crate keeps the example simple by focusing on structured logs and `#[instrument]`, which are still the first operational habits teams need before they move on to spans, exporters, and usage tracking.
